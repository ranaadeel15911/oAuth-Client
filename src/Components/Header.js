...
const Header = () => {
    ...
    const getUser = async () => {
        try {
            const response = await axios.get("http://o-auth-server-kappa.vercel.app/login/sucess", { withCredentials: true });
...
from here we will get req.user */            
    ...
    // logoout
    const logout = ()=>{
        window.open("http://o-auth-server-kappa.vercel.app/logout","_self")
    }
